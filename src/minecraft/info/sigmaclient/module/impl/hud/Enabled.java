...
public class Enabled extends Module {
	...
	void drawLigmaHUD(EventRenderHUD e, String colorMode){
		...
		float offset = Client.fm.getFont("SFR 11").getWidth(name);
		RenderingUtil.rectangle(2, 1, 64, 14, Colors.getColor(0, t ? (int)TabGUI.opacity.getX() : 200)); 	
		Client.fm.getFont("SFB 11").drawStringWithShadow(name, 3, 4,
				Colors.getColor(255, t ? (int)TabGUI.opacity.getX() + 64 : 232));
		Client.fm.getFont("SFB 7").drawStringWithShadow("v4.69", 3f + offset, 3.5F,
	...
	void drawJigsawHUD(EventRenderHUD e){
		mc.fontRendererObj.drawStringWithShadow("4.69", 100, 1, Colors.getColor(200));
		...
		for (Module module : modules) {
			...
			case"Sigma":
				...
				case "Rainbow":
					...
					color = new Color(rainB.getRed(), rainB.getGreen(), rainB.getBlue(), 250).getRGB();
					switch(fontMode){
					case"Sigma":
						RenderingUtil.rectangle(x-2, y-4.3, x-1, y+6.7, color);
						if(module != modules.get(0))
				...
				case"Sigma":
					...
					case"Sigma":
						sigmaFont.drawStringWithShadow(suffix, x + sigmaFont.getWidth(module.getName()) - 2, y - 1,
								Colors.getColor(Colors.getColor(180)));
						break;
					case"Vanilla":
